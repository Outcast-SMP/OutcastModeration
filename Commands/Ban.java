...
public class Ban implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        String errorMsg = new ConfigData(OutcastModeration.getInstance().chatMessages)
                .getStringFromConfig("chat.error.ban.incorrect-command-use");
        ...
        if (!new Punish(playerName).ban(ply, banTime, reason)) {
            new Chat(ply, Format.formatString(new ConfigData(OutcastModeration.getInstance().chatMessages)
                    .getStringFromConfig("chat.error.ban-message"), "#IGN", playerName)).message(true);
        }

        new Chat(ply, Format.formatString(new ConfigData(OutcastModeration.getInstance().chatMessages)
                .getStringFromConfig("chat.success.ban-message"), "#IGN", playerName)).message(true);
        return true;
    }
